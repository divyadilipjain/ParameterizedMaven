...
public class script extends IteratingVUserScript {
	...
	public void run() throws Exception {
		testingType=getSettings().get("testingType");
		...
		if(testingType.equalsIgnoreCase("Adhoc"))
		{
			...
			variables.setTestingType(testingType);
			adhocTesting();
		}
		else if(testingType.equalsIgnoreCase("release"))
		{
			...
			testCaseIds=getSettings().get("testCaseIds");
			if(testCaseIds.equalsIgnoreCase("NA"))
			{
				testCaseIds="";
			}
			releaseTesting();
	...
	//	DBUtill.trackId=Integer.parseInt(trackId.toString());
		...
		final Map<String, Set<String>> testCasesMap = DBUtill.getTestCaseNamesForAdhocTesting(appId,testCaseIds);
		testDataMap=applicationDB.getTestDataforAdhoc(appId, testCaseIds);
